...
class KiTS23Dataset(Dataset):
    ...
    def _sample_patch(self, image, label):
        """Sample random patch from volume with aggressive positive sampling"""
        d, h, w = image.shape
        ...
        # Random crop
        if self.is_train:
            # aggressive sampling: 70% chance to center on tumor/cyst if present
            if np.random.rand() < 0.7:
                # Find tumor (2) or cyst (3) voxels
                ...
                if len(target_indices) > 0:
                    ...
                    w_start = np.clip(center[2] - pw // 2, 0, w - pw)
                     ...
                     # Fallback to kidney if no tumor/cyst
                    target_indices = np.argwhere(label == 1)
                    if len(target_indices) > 0:
                        center = target_indices[np.random.randint(len(target_indices))]
                        d_start = np.clip(center[0] - pd // 2, 0, d - pd)
                        h_start = np.clip(center[1] - ph // 2, 0, h - ph)
...
class HybridLoss(nn.Module):
    ...
    def __init__(self, ds_weights=[1.0, 0.5, 0.25]):
        super().__init__()
        # Class weights to handle imbalance (Background, Kidney, Tumor, Cyst)
        # Register as buffer so it moves to device with the model
        class_weights = torch.tensor([0.2, 1.0, 4.0, 4.0])
        self.register_buffer('class_weights', class_weights)
        self.ds_weights = ds_weights
    
    def dice_loss(self, pred, target, smooth=1e-6):
        """Dice loss for multi-class with weighting"""
        ...
        # Apply class weights to Dice score
    ...
    def forward(self, outputs, target):
        ...
        for i, pred in enumerate(predictions):
            ...
            ce_loss = F.cross_entropy(pred, target_resized.squeeze(1), weight=self.class_weights)
