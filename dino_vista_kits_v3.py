...
"""
DINO-VISTA-KiTS v3: Complete Implementation
============================================
Foundation Model-Enhanced 3D Kidney Tumor Segmentation

Version 3 Changes:
- FIXED: Added missing 'cyst' class to EVAL_CONFIG hec_classes
- FIXED: Report generation now includes all HEC classes including cyst
- This fixes the 0.0 dice score issue for tumor/cyst evaluation
...
SCRIPT_NAME = Path(__file__).stem  # "dino_vista_kits_v3"
...
if QUICK_RUN:
    print("⚡ QUICK_RUN MODE v3 - Memory-optimized config")
    ...
    MODEL_CONFIG = {
        ...
        # Encoder - smaller for memory efficiency
        "init_filters": 16,           # Reduced from 24
        "blocks_down": [1, 2, 2, 2],
        "blocks_up": [1, 1, 1],
        
        # Multi-scale aggregation - smaller
        "aggregation_channels": 64,    # Reduced from 96
        "aggregation_scales": [2, 3, 4],
        
        # Dual-branch decoder - smaller embedding
        "class_embed_dim": 64,         # Reduced from 96
        "num_attention_heads": 4,
        "dropout": 0.1,
        
        # Training patches - SMALLER to fit in memory
        "patch_size": [64, 64, 64],    # Reduced from 96x96x96 (8x less voxels!)
    }
    ...
    TRAIN_CONFIG = {
        ...
        "use_amp": True,
        "val_interval": 2,
        "val_sw_batch_size": 1,        # Reduced from 2
        "val_overlap": 0.5,
        "save_interval": 5,
        "keep_best_n": 2,
...
EVAL_CONFIG = {
    ...
    "hec_classes": {
        ...
        "cyst": {"labels": (3,), "tolerance_mm": 2.0},  # v3 FIX: Added missing cyst class!
    },
}
...
def generate_report(metrics: Dict[str, float], output_path: str = None) -> str:
    ...
    for hec_name in ["kidney", "masses", "tumor", "cyst"]:  # v3 FIX: Added cyst to report
    ...
    # v3 FIX: Add Mean(K,T,C) summary like script.py
    ktc_classes = ["kidney", "tumor", "cyst"]
    ktc_dices = [metrics.get(f"hec_{c}_dice", 0) for c in ktc_classes if f"hec_{c}_dice" in metrics]
    ktc_nsds = [metrics.get(f"hec_{c}_surface_dice", 0) for c in ktc_classes if f"hec_{c}_surface_dice" in metrics]
    if ktc_dices:
        report.append(f"\n  {'Mean(K,T,C)':10s}:")
        report.append(f"    Dice: {np.mean(ktc_dices):.4f}")
        report.append(f"    NSD:  {np.mean(ktc_nsds):.4f}")
