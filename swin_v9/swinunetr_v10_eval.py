...
Usage
-----
  python swinunetr_v10_eval.py                              # auto-find best_final.pth
  python swinunetr_v10_eval.py --checkpoint path/to/ckpt   # explicit checkpoint
  python swinunetr_v10_eval.py --split test                 # test | val | all
  python swinunetr_v10_eval.py --no-save-nifti             # skip NIfTI export

Outputs  (./output/swin_v10/predict_export/)
...
def load_model(checkpoint_path: Path, config: dict, device: torch.device):
    ...
    # Apply S2 LoRA adapters so the state-dict keys match the checkpoint if LoRA is used
...
def run_inference(model, cases, config, device, pred_dir: Path, save_nifti: bool = True):
    ...
    for d in tqdm(cases, desc="Inference"):
        case_id = d["case_id"]
        try:
            # Load original NIfTI for shape information
            orig_nii = nib.load(d["image"])
            lbl_nii = nib.load(d["label"])
            lbl_gt_orig = lbl_nii.get_fdata().astype(np.int32)
            
            # Load resampled volume from cache (or online resampled)
            ...
            pred_res = torch.argmax(logits, dim=1).squeeze(0).cpu().numpy().astype(np.int32)
            
            # Resample predicted mask back to original shape
            zoom_factors_back = [o / p for o, p in zip(lbl_gt_orig.shape, pred_res.shape)]
            pred = zoom(pred_res, zoom_factors_back, order=0).astype(np.int32) # nearest neighbor
            
            # Post-process at original scale
            pred = postprocess(pred, config)

            # Compute metrics on original scale
            dice, iou, prec, rec = compute_metrics(pred, lbl_gt_orig)
            row = {"case_id": case_id}
...
def fig_slice_overlay(case_dict: dict, pred_path: Path, label: str,
                      out_path: Path):
    # Load original un-resampled volume and label for clean overlays
    img_nii = nib.load(case_dict["image"])
    lbl_nii = nib.load(case_dict["label"])
    img = img_nii.get_fdata().astype(np.float32)
    lbl_gt = lbl_nii.get_fdata().astype(np.int32)
    
    # Clip Hounsfield Units to [-175, 250] for optimal visualization contrast
    img = np.clip(img, -175.0, 250.0)
...
def make_slice_figures(df: pd.DataFrame, cases: list, pred_dir: Path,
    ...
    for group_name, ids in groups:
        for cid in ids:
            ...
            try:
                fig_slice_overlay(case_map[cid], pred_path, label, out)
...
def main():
    ...
    ap.add_argument("--output-dir", default="./output/swin_v10",
