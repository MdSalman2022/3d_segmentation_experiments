"""
KiTS21/23 Kidney Tumor Segmentation
===================================
Complete 3D segmentation pipeline using MONAI.
Based on KiTS21 nnUNet baseline approach.

Dataset: ./kits23/dataset
Classes: Background (0), Kidney (1), Tumor (2), Cyst (3)

Usage:
    python kits21_segmentation.py --mode train --epochs 100
    python kits21_segmentation.py --mode test --num_cases 5 --epochs 10
    python kits21_segmentation.py --mode predict --case case_00000
    python kits21_segmentation.py --mode evaluate --predictions_dir ./output
"""
...
def predict(case_id: str, model=None, save_prediction: bool = True):
    ...
    if save_prediction:
        ...
        # Resize if shapes don't match (e.g. inference on resampled image)
        if prediction.shape != orig_nii.shape:
            print(f"Resizing prediction from {prediction.shape} to {orig_nii.shape}")
            from monai.transforms import Resize
            resizer = Resize(spatial_size=orig_nii.shape, mode="nearest")
            # Add channel dim: (H, W, D) -> (1, H, W, D)
            pred_tensor = torch.from_numpy(prediction).unsqueeze(0)
            pred_tensor = resizer(pred_tensor)
            prediction = pred_tensor.squeeze(0).numpy().astype(np.uint8)
...
def evaluate_case(pred_path: str, gt_path: str) -> Dict[str, Dict[str, float]]:
    ...
    # Handle shape mismatch (e.g. if prediction was on resampled image)
    if pred_seg.shape != gt_seg.shape:
        # print(f"Shape mismatch: {pred_seg.shape} vs {gt_seg.shape}. Resizing prediction...")
        from monai.transforms import Resize
        # Note: sitk returns (D,H,W), monai Resize takes spatial size
        resizer = Resize(spatial_size=gt_seg.shape, mode="nearest")
        # Add channel dim
        pred_tensor = torch.from_numpy(pred_seg).unsqueeze(0)
        pred_tensor = resizer(pred_tensor)
        pred_seg = pred_tensor.squeeze(0).numpy().astype(np.uint8)
...
def get_versioned_path(directory: Union[str, Path], filename: str, extension: str) -> Path:
    """Get the next versioned filepath to avoid overwriting."""
    directory = Path(directory)
    directory.mkdir(parents=True, exist_ok=True)
    
    # Try the base filename first (v1 implicit)
    base_v1 = directory / f"{filename}_v1{extension}"
    if not base_v1.exists():
        return base_v1
        
    i = 2
    while True:
        candidate = directory / f"{filename}_v{i}{extension}"
...
def evaluate_predictions(
    ...
    # Calculate Mean of Interest (Kidney, Tumor, Cyst)
    target_classes = ["kidney", "tumor", "cyst"]
    if all(c in mean_metrics for c in target_classes):
        ktc_dice = np.mean([mean_metrics[c]["dice"] for c in target_classes])
        ktc_sd = np.mean([mean_metrics[c]["surface_dice"] for c in target_classes])
        print(f"{'-'*35}")
        print(f"{'Mean(K,T,C)':<10} {ktc_dice:>10.4f} {ktc_sd:>15.4f}")
    
    print(f"{'='*60}")
    
    if output_csv is None:
        output_csv = get_versioned_path(predictions_dir, "evaluation_results", ".csv")
    ...
    with open(output_csv, "w") as f:
        ...
        # Save the KTC mean as well in a separate mini-report or just logged above
        # Since CSV structure is fixed by HEC_NAMES, we won't break it, 
        # but we already printed it.
