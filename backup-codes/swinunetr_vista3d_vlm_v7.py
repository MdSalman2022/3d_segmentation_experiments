...
class KiTS23Dataset(Dataset):
    ...
    def _sample_patch(
        ...
        if self.is_train:
            ...
            # Val: return the FULL volume (padded if necessary)
            # We no longer crop to patch_size because sliding_window_inference needs the whole volume
            # to generate full-size pseudo-labels that match the NIfTI coordinate space.
            patch_img = img
            patch_lbl = lbl
...
def validate(model, loader, loss_fn, device):
    ...
    # Get patch size from the dataset (assuming loader.dataset has patch_size)
