"""
VLM-in-the-Loop 3D Kidney Segmentation — V6
============================================
Architecture:
  - SwinUNETR backbone (MONAI pretrained, natively 3D, replaces DINOv2+CNN)
  ...
  - VLM-in-the-Loop Training (VITL) — Option B:
      VLM inspects failure cases → generates corrective pseudo-labels →
      pseudo-labels added to training pool with confidence weighting.
      This directly replaces the human expert correction loop from VISTA3D.

Key improvement over V5:
  V5: VLM only adjusts loss weights/sampling (spectator)
  V6: VLM generates corrected pseudo-labels (participant)
      → Model actually learns from VLM's segmentation corrections

Dataset: KiTS23 (Kidney Tumor Segmentation)
Classes: 0=Background, 1=Kidney, 2=Tumor, 3=Cyst
GPU:     RTX 5090 32GB  |  Qwen3-4B via Ollama (~8GB)

Usage:
    python swinunetr_vista3d_vlm.py --mode quick_test
    python swinunetr_vista3d_vlm.py --mode full
    python swinunetr_vista3d_vlm.py --mode full --resume
    python swinunetr_vista3d_vlm.py --mode full --no-vlm        # ablation: MGPL only
    python swinunetr_vista3d_vlm.py --mode full --no-mgpl       # ablation: VITL only
"""
...
def get_config(mode: str) -> dict:
    base = {
        ...
        "use_vlm": True,
        "vlm_backend": "ollama",
        "vlm_model": "qwen3-vl:4b",  # vision-language variant — required for image analysis
    ...
    _qt = {
        ...
        "val_every_s2": 1,
        "output_dir": "./output/vlm_loop_v6_quick",
    }
    ...
    if mode == "quick_test_vlm":
        ...
        base["use_vlm"] = True
        base["use_mgpl"] = True
        base["output_dir"] = "./output/vlm_loop_v6_quick_VLM"
    elif mode == "quick_test_novlm":
        # Ablation B: MGPL only, VLM DISABLED — pure baseline
        base.update(_qt)
        base["use_vlm"] = False
        base["use_mgpl"] = True
        base["output_dir"] = "./output/vlm_loop_v6_quick_noVLM"
...
"""
    ...
    def _call_ollama(self, prompt: str, images_b64: List[str]) -> str:
        ...
        try:
            r = requests.post(f"{self.api_url}/api/generate", json=payload, timeout=120)
            r.raise_for_status()
