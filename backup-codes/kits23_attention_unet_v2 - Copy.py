...
CONFIG = {
    ...
    "model": {
        ...
        "channels": (32, 64, 128, 256, 256),  # Reduced channels for memory
        "strides": (2, 2, 2, 2),
        "num_res_units": 0,  # Disabled for memory savings
        "attention_ratio": 16,
        "spatial_kernel": 7,
        "dropout": 0.1,
    },
    
    # Training settings - V2 Improved
    "training": {
        "num_epochs": 200,  # More epochs with better learning rate schedule
        "quick_epochs": 5,
        "batch_size": 1,
        "num_samples": 4,  # Reduced for memory
        "learning_rate": 2e-4,
        "use_amp": True,  # Mixed precision training for memory savings
        ...
        # Class weights for loss (CRITICAL for tumor/cyst)
        # Higher weight = more focus on that class
        "class_weights": [0.2, 1.0, 4.0, 6.0],  # bg, kidney, tumor, cyst
        
        # Positive/negative sampling ratio (oversample tumor/cyst regions)
        "pos_neg_ratio": (3, 1),  # 3:1 ratio for positive patches
    },
    
    # Data settings - MEMORY OPTIMIZED
    "data": {
        "patch_size": (96, 96, 48),  # Smaller patch for memory
        "spacing": (2.0, 2.0, 2.0),  # Coarser spacing to reduce volume size
        "intensity_min": -79,  # Kidney CT window
        "intensity_max": 304,
...
class CombinedLossV2(nn.Module):
    ...
    def __init__(self, class_weights: List[float] = None):
        ...
        # Tversky loss for handling small structures
        self.tversky = TverskyLoss(
            alpha=0.3,  # Less penalty for false positives
            beta=0.7,   # More penalty for false negatives (missing tumors)
