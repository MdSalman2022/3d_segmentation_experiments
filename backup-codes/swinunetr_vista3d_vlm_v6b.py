"""
VLM-in-the-Loop 3D Kidney Segmentation — V6b
=============================================
Same as V6 with VLM upgrade:
  - LLaVA-Med v1.5 replaces Qwen3-VL as the critic model
    (z-uo/llava-med-v1.5-mistral-7b_q8_0:latest — medically fine-tuned, 8.3GB)
  - Startup VLM health check: training aborts with a clear error if
    Ollama is not running or the model is not loaded, preventing
    silent zero-correction epochs as in V6.
  - Output dir: output/vlm_loop_v6b (separate from V6)
...
Architecture (unchanged from V6):
  ...
  - VLM-in-the-Loop Training (VITL) — Option B:
      LLaVA-Med inspects worst cases → generates corrective pseudo-labels
      → pseudo-labels added to training pool with confidence weighting.

Dataset: KiTS23 (Kidney Tumor Segmentation)
Classes: 0=Background, 1=Kidney, 2=Tumor, 3=Cyst
GPU:     RTX 5090 32GB  |  LLaVA-Med via Ollama (~8.3GB)

Prerequisites:
    ollama serve
    ollama pull z-uo/llava-med-v1.5-mistral-7b_q8_0:latest

Usage:
    python swinunetr_vista3d_vlm_v6b.py --mode quick_test
    python swinunetr_vista3d_vlm_v6b.py --mode full
    python swinunetr_vista3d_vlm_v6b.py --mode full --resume
    python swinunetr_vista3d_vlm_v6b.py --mode full --no-vlm
    python swinunetr_vista3d_vlm_v6b.py --mode full --no-mgpl
"""
...
def get_config(mode: str) -> dict:
    base = {
        ...
        "output_dir": "./output/vlm_loop_v6b",
        # Model
        "num_classes": 4,
        "feature_dim": 256,
        ...
        "critic_every_k_epochs": 5,
        "critic_top_n_worst": 10,
        "critic_num_slices": 3,
        "pseudo_label_confidence_threshold": 0.50,  # only use corrections above this
        "pseudo_label_loss_weight": 0.50,  # trust VLM labels 50% as much as GT
        "pseudo_label_max_pool_size": 200,  # max pseudo-label entries stored
        # Split
        "test_cases": 40,
        "seed": 42,
    }
    ...
    _qt = {
        ...
        "val_every_s2": 1,
        "output_dir": "./output/vlm_loop_v6b_quick",
    }
    ...
    if mode == "quick_test_vlm":
        ...
        base["use_vlm"] = True
        base["use_mgpl"] = True
        base["output_dir"] = "./output/vlm_loop_v6b_quick_VLM"
    elif mode == "quick_test_novlm":
        # Ablation B: MGPL only, VLM DISABLED — pure baseline
        base.update(_qt)
        base["use_vlm"] = False
        base["use_mgpl"] = True
        base["output_dir"] = "./output/vlm_loop_v6b_quick_noVLM"
...
class VLMLoopSegNet(nn.Module):
    ...
    def __init__(
        ...
        trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"\n{'='*55}")
        print(f"VLMLoopSegNet V6 | Total: {total:,} | Trainable: {trainable:,}")
...
# ============================================================================
# VLM-IN-THE-LOOP CRITIC — OPTION B: PSEUDO-LABEL GENERATION
# ============================================================================


class VLMPseudoLabelCritic:
    """
    VLM-in-the-Loop Training (VITL) — Option B.

    VISTA3D used doctor click corrections to refine segmentation.
    We replace the doctor with a VLM:

      Model predicts → VLM inspects failure cases → VLM generates
      corrective pseudo-labels → pseudo-labels added to training pool
      with confidence weighting → model learns from VLM corrections.

    The VLM outputs structured spatial corrections:
      - Missed regions (false negatives): "add class X in this region"
      - Over-segmented regions (false positives): "remove class X here"
      - Class confusion: "this is cyst, not tumor"

    Corrections are filtered by confidence threshold before use.
    """
    ...
    def generate_corrections(
        ...
        epoch: int,
        val_cases: List[dict],
        predictions: Dict[str, np.ndarray],  # {case_id: pred (D,H,W) int}
        ground_truths: Dict[str, np.ndarray],  # {case_id: gt  (D,H,W) int}
        images: Dict[str, np.ndarray],  # {case_id: img (D,H,W) float}
    ) -> Dict[str, dict]:
        """
        Main entry point.
        Returns {case_id: {"pseudo_label": np.ndarray, "confidence": float}}
        Only returns entries where confidence >= self.conf_threshold.
        """
        ...
        print(
            f"\n  [VLM] Generating pseudo-label corrections for "
        ...
        for case_info in worst_cases:
            ...
            try:
                ...
                if correction and correction["confidence"] >= self.conf_threshold:
                    ...
                    print(
                        f"    ✓ {cid}: confidence={correction['confidence']:.2f}, "
                        f"corrections={correction['num_corrections']}"
                    )
                elif correction:
                    print(
        ...
        print(
            f"  [VLM] Applied {len(results)}/{len(worst_cases)} corrections "
    ...
    def _correct_single_case(
        ...
        """
        Ask the VLM to inspect this case and return a corrected pseudo-label.
        The correction starts from the model's prediction and applies VLM edits.
        """
        # Build rendered images for VLM
        images_b64 = self._render_case(case_id, epoch, image, prediction, ground_truth)

        # Build prompt asking for spatial corrections
        prompt = self._build_correction_prompt(
            case_id, case_info, prediction, ground_truth
        )
        ...
        correction_data = self._parse_correction_response(response)
        if correction_data is None:
            return None

        # Apply corrections to prediction to create pseudo-label
        pseudo_label, num_corrections = self._apply_corrections(
            prediction.copy(), ground_truth, correction_data
        )
    ...
    def _build_correction_prompt(
        ...
        # Compute error statistics to help VLM
        ...
        kidney_fp = int(((prediction == 1) & (ground_truth != 1)).sum())

        # Find slice ranges with errors
        tumor_error_slices = np.where(
            ((ground_truth == 2) & (prediction != 2)).any(axis=(1, 2))
        )[0].tolist()[:5]

        return f"""You are an expert radiologist reviewing kidney CT segmentation errors.

Case: {case_id}
Volume shape: {D}×{H}×{W} voxels
Current Dice — Background:{dice[0]:.3f} Kidney:{dice[1]:.3f} Tumor:{dice[2]:.3f} Cyst:{dice[3]:.3f}

Error statistics:
- Tumor false negatives (missed tumor voxels): {tumor_fn:,}
- Tumor false positives (wrong tumor label): {tumor_fp:,}
- Cyst false negatives (missed cyst voxels): {cyst_fn:,}
- Kidney false positives: {kidney_fp:,}
- Slices with tumor errors: {tumor_error_slices}

The images show (left to right):
  Panel 1: Raw CT slice (grayscale)
  Panel 2: Ground truth (Green=Kidney, Red=Tumor, Blue=Cyst)
  Panel 3: Model prediction overlay
  Panel 4: Error map (Red=FP wrong label, Blue=FN missed)

Based on the visual errors, provide spatial correction instructions.
Respond ONLY with this exact JSON structure (no other text):
{{
  "confidence": <float 0.0-1.0 — your confidence in these corrections>,
  "dominant_error": "<missed_tumor|tumor_boundary|class_confusion|over_segmentation|cyst_missed>",
  "corrections": [
    {{
      "type": "<add|remove|reclassify>",
      "from_class": <0-3 or null>,
      "to_class": <0-3>,
      "region": "<upper_pole|lower_pole|central|medial|lateral|whole>",
      "z_range": [<start_slice>, <end_slice>],
      "description": "<brief description of the correction>"
    }}
  ],
  "severity": "<mild|moderate|severe>",
  "primary_failure": "<recall|precision|boundary|class_confusion>"
}}

Only include corrections you are confident about from the images.
If the segmentation looks correct, set confidence to 0.0 and corrections to [].
"""
    ...
    def _apply_corrections(
        ...
        """
        Apply VLM correction instructions to the prediction.
        This is the key step: VLM judgment directly modifies the pseudo-label.

        Strategy:
          - For high-confidence corrections in regions with clear GT errors,
            apply the GT label in that region (trust GT more than model)
          - For boundary corrections, use a blend guided by both GT and model
          - Track total number of voxels corrected
        """
        ...
        for corr in corrections:
            ...
            z_range = corr.get("z_range", [0, D])
            region_str = corr.get("region", "whole")

            if to_class is None or not (0 <= to_class <= 3):
                continue

            z_start = max(0, int(z_range[0]))
            z_end = min(D, int(z_range[1]) + 1)

            # Region mask (spatial)
            region_mask = self._region_str_to_mask(region_str, D, H, W)
            slice_mask = np.zeros((D, H, W), dtype=bool)
            slice_mask[z_start:z_end] = True
            combined_mask = region_mask & slice_mask

            if corr_type == "add":
                # Add class to_class where GT agrees but model missed
                gt_agrees = (ground_truth == to_class) & combined_mask
                if gt_agrees.sum() > 10:  # minimum region size guard
                    pseudo_label[gt_agrees] = to_class
                    num_corrected += int(gt_agrees.sum())

            elif corr_type == "remove":
                from_class = corr.get("from_class")
                if from_class is None:
                    continue
                # Remove class from_class where GT says it's wrong
                wrong_label = (pseudo_label == from_class) & combined_mask
                gt_disagrees = wrong_label & (ground_truth != from_class)
                if gt_disagrees.sum() > 10:
                    # Replace with GT label
                    pseudo_label[gt_disagrees] = ground_truth[gt_disagrees]
                    num_corrected += int(gt_disagrees.sum())

            elif corr_type == "reclassify":
                from_class = corr.get("from_class")
                if from_class is None:
                    continue
                # Reclassify: change from_class → to_class where GT agrees
                reclassify_mask = (pseudo_label == from_class) & combined_mask
                gt_agrees_new = reclassify_mask & (ground_truth == to_class)
                if gt_agrees_new.sum() > 5:
                    pseudo_label[gt_agrees_new] = to_class
                    num_corrected += int(gt_agrees_new.sum())

        return pseudo_label, num_corrected

    def _region_str_to_mask(
        self, region_str: str, D: int, H: int, W: int
    ) -> np.ndarray:
        """Convert region descriptor to 3D boolean mask."""
        mask = np.ones((D, H, W), dtype=bool)
        r = region_str.lower()
        if r == "whole":
            return mask
        if "upper" in r or "superior" in r:
            mask[: D // 2] = False
        elif "lower" in r or "inferior" in r:
            mask[D // 2 :] = False
        if "medial" in r:
            mask[:, :, W // 2 :] = False
        elif "lateral" in r:
            mask[:, :, : W // 2] = False
        if "central" in r:
            d4, h4, w4 = D // 4, H // 4, W // 4
            outer = np.ones((D, H, W), dtype=bool)
            outer[d4 : 3 * d4, h4 : 3 * h4, w4 : 3 * w4] = False
            mask &= ~outer
        return mask
    ...
    def _render_case(
        ...
        # Find slices richest in tumor errors
    ...
    def _call_ollama(self, prompt: str, images_b64: List[str]) -> str:
        ...
        for endpoint, payload, key in [
            ...
            try:
                r = requests.post(
                    f"{self.api_url}/{endpoint}", json=payload, timeout=180
                )
                r.raise_for_status()
                text = key(r.json()).strip()
                if text:
                    return text
            except Exception as e:
                print(f"    [VLM] {endpoint} error: {e}")
        print(
...
def _check_vlm_health(config: dict, abort_on_fail: bool = True) -> bool:
    ...
    for attempt_model in models_to_try:
        ...
        text_only_ok = False
        for endpoint, payload, extract in [
            (
                "api/chat",
                {
                    "model": matched,
        ...
        # Step 3: Test with image (use 64x64 red square — avoids 1px crash in vision encoders)
        import base64, io
        ...
        response_text = ""
        for endpoint, payload, extract in candidates:
            try:
                r2 = requests.post(f"{api_url}/{endpoint}", json=payload, timeout=300)
                r2.raise_for_status()
                response_text = extract(r2.json()).strip()
                if response_text:
        ...
        if response_text:
            # Success! Update config if we switched models
            ...
            print(
                f"  ✓ VLM ready | model='{matched}' | endpoint='{working_endpoint}' | response='{response_text[:50]}'"
            )
            return True
...
def train(config: dict, resume: bool = False, skip_stage1: bool = False):
    ...
    logger.info("=" * 65)
    logger.info("VLM-in-the-Loop 3D Segmentation — V6 (Option B: Pseudo-Labels)")
    ...
    print(f"\n{'='*65}")
    print("VLM-in-the-Loop 3D Segmentation — V6 (Option B: Pseudo-Labels)")
    print(f"{'='*65}")
    ...
    # ── VLM Critic ──
    critic = VLMPseudoLabelCritic(config) if config["use_vlm"] else None
    if critic:
        print(f"✓ VLM Critic: {config['vlm_backend']}:{config['vlm_model']}")
        print(
    ...
    for epoch in range(start_ep if not _skip_stage1 else n_s1, n_s1):
        ...
        if (
            ...
            # Build prediction/GT/image dicts from per_case results
            preds_dict = {pc["case_id"]: pc["pred"] for pc in last_val["per_case"]}
            gts_dict = {pc["case_id"]: pc["gt"] for pc in last_val["per_case"]}
            imgs_dict = {pc["case_id"]: pc["img"] for pc in last_val["per_case"]}
            ...
            if corrections:
                ...
                # Refresh dataset to include new pseudo-labels
                train_loader.dataset.refresh_pseudo_labels()
                vlm_summary = {
    ...
    summary = {
        "version": "VLMLoopSegNet_V6_OptionB",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="VLM-in-the-Loop 3D Segmentation V6 (Option B: Pseudo-Labels)"
    )
    parser.add_argument(
        "--mode",
        default="full",
