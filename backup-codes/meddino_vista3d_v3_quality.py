"""
MedDINO-VISTA3D V3 Quality Edition
===================================

Based on proven V1 architecture with safe optimizations.
...
- 50% cyst-focused aggressive sampling

OPTIMIZATIONS (V3):
- Faster data loading: 4 workers (vs 2 in V1)
- Reduced warmup: 5 epochs (vs 10 in V1)
- Optional post-processing for final validation

PERFORMANCE (Quick Test):
- Mean Dice: 0.49 | Cyst Dice: 0.064
- Faster than V2 without quality loss

Usage:
    python meddino_vista3d_v3_quality.py --mode quick_test
    python meddino_vista3d_v3_quality.py --mode full
"""
...
def get_config(mode):
    ...
    base = {
        ...
        "output_dir": "./output/meddino_vista3d_v3_quality",
...
# ============================================================================
# LOSS FUNCTIONS - Cyst-Aggressive
# ============================================================================

class TverskyLoss(nn.Module):
    """Tversky Loss - heavily penalizes false negatives (missing cysts)"""
...
class EnhancedHybridLoss(nn.Module):
    """Phase 1 Enhanced Loss: Tversky + Focal + Dice"""
    
    def __init__(self):
        super().__init__()
        # AGGRESSIVE class weights: BG, Kidney, Tumor, Cyst
        w = torch.tensor([0.1, 1.0, 12.0, 20.0])  # Cyst weight increased to 20!
...
def save_epoch_results(output_dir, stage, epoch, train_loss, val_loss, metrics, epoch_time, is_best=False):
    ...
    if history_file.exists():
        try:
            ...
            print(f"⚠ Warning: Corrupted history file, creating new one. Error: {e}")
            history = {"epochs": [], "best_dice": 0.0, "total_time_hours": 0.0}
    else:
        history = {"epochs": [], "best_dice": 0.0, "total_time_hours": 0.0}
    
    history["epochs"].append(epoch_data)
    if is_best:
        history["best_dice"] = float(metrics["dice"][1:].mean())
    history["total_time_hours"] = round(sum(e["epoch_time_seconds"] for e in history["epochs"]) / 3600, 2)
...
def save_final_evaluation(output_dir, best_dice, total_params, config, total_time):
    ...
    final_eval = {
        "timestamp": datetime.now().isoformat(),
        "model": "MedDINO-VISTA3D V3 Quality Edition",
        "best_overall_dice": float(best_dice),
        "total_parameters": total_params,
        "total_training_time_hours": round(total_time / 3600, 2),
        "configuration": {
            "encoder": config["dinov2_backbone"],
    ...
    print(f"   - training_history.json (epoch-by-epoch with timing)")
    print(f"   - final_evaluation.json (summary)")
...
def save_checkpoint(output_dir, epoch, stage, model, optimizer, scheduler, best_dice, scaler):
    ...
    latest_path = output_dir / f"checkpoint_{stage}_latest.pth"
    torch.save(checkpoint, latest_path)
    
    # Clean up old checkpoints (keep only last 3)
    checkpoints = sorted(output_dir.glob(f"checkpoint_{stage}_epoch_*.pth"))
...
def load_checkpoint(output_dir, stage, model, optimizer, scheduler, scaler, device):
    ...
    print(f"✓ Resumed from epoch {checkpoint['epoch']}, stage: {checkpoint['stage']}")
...
def train(config, resume=False):
    print("=" * 70)
    print("MedDINO-VISTA3D V3 QUALITY EDITION")
    print("Architecture: V1 (proven) + Speed optimizations")
    print(f"Encoder: {config['dinov2_backbone']} | Workers: 4 | Warmup: 5 epochs")
    print("=" * 70)
    ...
    loss_fn = EnhancedHybridLoss().to(device)
    
    best_dice = 0.0
    total_training_start = time.time()
    checkpoint_freq = config.get('checkpoint_freq', 5)  # Save every 5 epochs
    ...
    if resume:
        ...
        if checkpoint:
            ...
            if start_epoch_stage1 >= config["num_epochs_stage1"]:
                print("Stage 1 already completed, skipping to Stage 2...")
                start_epoch_stage1 = config["num_epochs_stage1"]
    
    for epoch in range(start_epoch_stage1, config["num_epochs_stage1"]):
