...
def create_table_image(output_dir: str):
    base_dir = Path("./")
    out_dir = base_dir / output_dir / "figures"
    out_dir.mkdir(parents=True, exist_ok=True)
    
    json_path = base_dir / output_dir / "fair_results.json"
    if not json_path.exists():
        json_path = base_dir / output_dir / "test_results.json"
        
    print(f"Loading results from: {json_path}")
    with open(json_path, 'r') as f:
        data = json.load(f)
        
    cases = data.get('cases', data.get('per_case', []))
    ...
    table = ax.table(
        ...
        colWidths=[0.1, 0.15, 0.25, 0.15, 0.2, 0.15]
    )
    ...
    plt.title("SwinUNETR V6b Performance Summary", fontsize=16, fontweight='bold', y=0.85)
    
    out_path = out_dir / "performance_table.png"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="output/vlm_loop_v6b_quick_VLM")
    args = parser.parse_args()
    create_table_image(args.output_dir)
