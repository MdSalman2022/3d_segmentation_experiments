...
class VISTA3DTrainer:
    ...
    def validate(self) -> Dict[str, float]:
        ...
        for batch in tqdm(self.val_loader, desc="Validating", leave=False):
            ...
            # Convert predictions to one-hot format for DiceMetric
            # DiceMetric expects: y_pred as one-hot [B, C, H, W, D], y as indices [B, 1, H, W, D]
            outputs_softmax = torch.softmax(outputs, dim=1)
            outputs_onehot = (outputs_softmax == outputs_softmax.max(dim=1, keepdim=True)[0]).float()
            self.dice_metric(outputs_onehot, labels)
        
        dice_per_class = self.dice_metric.aggregate()
        results = {
            "mean_loss": total_loss / len(self.val_loader),
            "mean_dice": dice_per_class.mean().item(),
            "dice_kidney": dice_per_class[0].item() if len(dice_per_class) > 0 else 0,
            "dice_tumor": dice_per_class[1].item() if len(dice_per_class) > 1 else 0,
            "dice_cyst": dice_per_class[2].item() if len(dice_per_class) > 2 else 0,
        }
        return results
