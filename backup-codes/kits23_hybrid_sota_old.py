...
Usage:
    python kits23_hybrid_sota.py --mode train --quick     # Quick test (5 epochs)
    python kits23_hybrid_sota.py --mode train             # Full training
    python kits23_hybrid_sota.py --mode inference
    python kits23_hybrid_sota.py --mode evaluate
"""
...
CONFIG = {
    ...
    "stage1": {
        ...
        "num_samples": 6,     # Balanced: more data variety, moderate memory
        "learning_rate": 1e-4,
        "cache_rate": 0.1,    # Lower cache for less RAM usage
    ...
    "stage2": {
        "spacing": (1.0, 1.0, 1.5),   # Balanced resolution
        "patch_size": (128, 128, 96), # Full-size patches
        "channels": (64, 128, 256, 512, 512),  # Full capacity
        "strides": (2, 2, 2, 2),
        "out_channels": 4,  # bg, kidney, tumor, cyst
        "epochs": 500,
        "quick_epochs": 5,
        "batch_size": 1,              # Keep at 1 to avoid OOM
        "num_samples": 4,             # Good variety
        "learning_rate": 1e-4,
        "deep_supervision": True,     # Enabled for better gradients
        "class_weights": [0.1, 0.3, 15.0, 12.0],  # Heavy tumor/cyst
        "num_workers": 4,
    },
...
# TRAINING FUNCTIONS
# ============================================================================
def train_stage1(config: dict, num_epochs: int = None):
    ...
    torch.save({"model": model.state_dict()}, checkpoint_dir / "stage1_last.pth")
...
def train_stage2(config: dict, num_epochs: int = None):
    ...
    torch.save({"model": model.state_dict()}, checkpoint_dir / "stage2_last.pth")
...
def run_inference(config: dict, use_ensemble: bool = True):
    ...
    print("\n" + "=" * 70)
    print("  INFERENCE" + (" (Ensemble)" if use_ensemble else ""))
    print("=" * 70)
    ...
    model = SCUNet(
        in_channels=1, out_channels=cfg["out_channels"],
        channels=cfg["channels"], strides=cfg["strides"],
        deep_supervision=False  # No DS for inference
    ).to(device)
    ...
    print(f"Using {len(checkpoints)} checkpoint(s)")
    
    data_dicts = get_data_dicts(config["kits23_dir"])
    transforms = get_stage2_transforms(config, training=False)
...
def main():
    ...
    parser.add_argument("--mode", choices=["train", "train_stage1", "train_stage2", 
                                           "inference", "evaluate", "all"], default="train")
    ...
    if args.mode in ["train", "train_stage1", "all"]:
        train_stage1(config, stage1_epochs)
    
    if args.mode in ["train", "train_stage2", "all"]:
        train_stage2(config, stage2_epochs)
